...
- **Node.js 22** — if not installed:
  ...
  ```
- **OpenShell CLI** (install from a pinned GitHub release via `gh` or a manually verified archive):
  ```bash
  gh release download --repo NVIDIA/OpenShell --pattern 'openshell-aarch64-unknown-linux-musl.tar.gz'
  tar xzf openshell-aarch64-unknown-linux-musl.tar.gz
