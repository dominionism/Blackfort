...
const { run, runCapture, sleep } = require("./runner");
...
function detectGpu() {
  ...
  try {
    const output = runCapture("nvidia-smi", [
  ...
  try {
    const nameOutput = runCapture("nvidia-smi", [
      "--query-gpu=name",
      "--format=csv,noheader,nounits",
    ], { ignoreError: true });
    if (nameOutput && nameOutput.includes("GB10")) {
      ...
      let totalMemoryMB = 0;
      try {
        const memLine = runCapture("free", ["-m"], { ignoreError: true });
        const match = memLine.match(/^Mem:\s+(\d+)/m);
        if (match) totalMemoryMB = parseInt(match[1], 10) || 0;
      } catch {}
      return {
        type: "nvidia",
        count: 1,
...
function startNimContainer(sandboxName, model, port = 8000) {
  ...
  run("docker", ["rm", "-f", name], { ignoreError: true, stdio: "ignore" });
...
function stopNimContainer(sandboxName) {
  ...
  run("docker", ["stop", name], { ignoreError: true, stdio: "ignore" });
  run("docker", ["rm", name], { ignoreError: true, stdio: "ignore" });
}
