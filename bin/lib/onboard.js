...
const { ROOT, SCRIPTS, run, runCapture, commandExists, sleep, spawn } = require("./runner");
...
function isOpenshellInstalled() {
  return commandExists("openshell");
}
...
function assertSafeSandboxName(name) {
  if (!SANDBOX_NAME_RE.test(name)) {
    console.error(
      "  Invalid sandbox name. Use 1-63 characters from: letters, numbers, dot, underscore, hyphen.",
    );
    process.exit(1);
  }
}

function startBackgroundCommand(command, args, env = {}) {
...
function upsertProvider(name, credential, baseUrl) {
  const createArgs = [
    "provider",
    "create",
    "--name",
  ...
  const createResult = spawn("openshell", createArgs);
  if (createResult.status === 0) {
    return true;
  }

  const updateResult = spawn("openshell", [
    "provider",
    "update",
    name,
    "--credential",
...
async function startGateway(gpu) {
  ...
  run("openshell", ["gateway", "start", ...gwArgs], { ignoreError: false });
  ...
  if (colimaSocket) {
    ...
    run("bash", [path.join(SCRIPTS, "fix-coredns.sh")], { ignoreError: true });
  }
...
async function createSandbox(gpu) {
  ...
  assertSafeSandboxName(sandboxName);
  ...
  const envArgs = [
    ...
    `NEMOCLAW_SECURITY_PROFILE=${process.env.NEMOCLAW_SECURITY_PROFILE || "prod-secure"}`,
  ];
  if (process.env.NEMOCLAW_ACCESS_PROXY_URL) {
    envArgs.push(`NEMOCLAW_ACCESS_PROXY_URL=${process.env.NEMOCLAW_ACCESS_PROXY_URL}`);
  }
  if (process.env.NVIDIA_API_KEY) {
    envArgs.push(`NVIDIA_API_KEY=${process.env.NVIDIA_API_KEY}`);
  }
  run("openshell", [...createArgs, "--", "env", ...envArgs, "nemoclaw-start"]);
...
async function setupNim(sandboxName, gpu) {
  ...
  const ollamaRunning = !!runCapture("curl", ["-sf", "http://localhost:11434/api/tags"], {
    ignoreError: true,
  });
  const vllmRunning = !!runCapture("curl", ["-sf", "http://localhost:8000/v1/models"], {
    ignoreError: true,
  });
  ...
  if (options.length > 1) {
    ...
    if (selected.key === "nim") {
      ...
      if (!ollamaRunning) {
        ...
        startBackgroundCommand("ollama", ["serve"], { OLLAMA_HOST: "0.0.0.0:11434" });
        sleep(2);
      }
...
async function setupInference(sandboxName, model, provider) {
  ...
  if (provider === "nvidia-nim") {
    if (!upsertProvider("nvidia-nim", `NVIDIA_API_KEY=${process.env.NVIDIA_API_KEY}`, "https://integrate.api.nvidia.com/v1")) {
      console.error("  Failed to configure nvidia-nim provider");
      process.exit(1);
    }
    run("openshell", ["inference", "set", "--provider", "nvidia-nim", "--model", model], {
      ignoreError: true,
      stdio: "ignore",
    });
  } else if (provider === "vllm-local") {
    if (!upsertProvider("vllm-local", "OPENAI_API_KEY=dummy", `${HOST_GATEWAY_URL}:8000/v1`)) {
      console.error("  Failed to configure vllm-local provider");
      process.exit(1);
    }
    run("openshell", ["inference", "set", "--provider", "vllm-local", "--model", model], {
      ignoreError: true,
      stdio: "ignore",
    });
  } else if (provider === "ollama-local") {
    if (!upsertProvider("ollama-local", "OPENAI_API_KEY=ollama", `${HOST_GATEWAY_URL}:11434/v1`)) {
      console.error("  Failed to configure ollama-local provider");
      process.exit(1);
    }
    run("openshell", ["inference", "set", "--provider", "ollama-local", "--model", model], {
      ignoreError: true,
      stdio: "ignore",
    });
  }
