...
function syncSnapshotBundleIntoSandbox(bundle: SnapshotBundle, sandboxName: string): void {
  execSandboxCommand(sandboxName, ["mkdir", "-p", SANDBOX_ARCHIVE_DIR]);
...
function syncArchive(sandboxName: string, archiveName: string, archivePath: string, destinationDir: string): void {
  ...
  execSandboxCommand(sandboxName, ["mkdir", "-p", destinationDir]);
  execSandboxCommand(sandboxName, ["tar", "-xf", sandboxArchivePath, "-C", destinationDir]);
}
...
function verifySandboxMigration(bundle: SnapshotBundle, sandboxName: string): void {
  ...
  const verification = {
    ...
    quarantineManifestPath: "/sandbox/.openclaw/.nemoclaw-quarantine/quarantine.json",
    expectQuarantineManifest: manifest.externalRoots.some((root) => root.kind === "skillsExtraDir"),
    roots: manifest.externalRoots.map((root) => ({
      id: root.id,
      kind: root.kind,
      sandboxPath: root.sandboxPath,
      bindings: root.kind === "skillsExtraDir" ? [] : root.bindings.map((binding) => ({
...
if (verification.expectQuarantineManifest && !fs.existsSync(verification.quarantineManifestPath)) {
  throw new Error(\`Missing quarantine manifest: \${verification.quarantineManifestPath}\`);
}
