...
function quarantineImportedState(
  preparedStateDir: string,
  config: OpenClawConfigDocument,
  manifest: SnapshotManifest,
): void {
  const quarantineDir = path.join(preparedStateDir, QUARANTINE_DIR_NAME);
  const quarantined: Array<Record<string, string>> = [];

  for (const entry of ["hooks", "extensions", "skills"]) {
    const sourcePath = path.join(preparedStateDir, entry);
    if (!existsSync(sourcePath)) {
      continue;
    }
    mkdirSync(quarantineDir, { recursive: true });
    const destinationPath = path.join(quarantineDir, entry);
  ...
  if (quarantined.length > 0) {
    mkdirSync(quarantineDir, { recursive: true });
    writeFileSync(
      path.join(quarantineDir, "quarantine.json"),
      JSON.stringify(
        {
