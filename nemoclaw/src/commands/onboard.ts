...
function detectOllama(): { installed: boolean; running: boolean } {
  const installed = testCommand("ollama", ["--version"]);
  const running = testCommand("curl", ["-sf", "http://localhost:11434/api/tags"]);
  return { installed, running };
}

function testCommand(command: string, args: string[]): boolean {
