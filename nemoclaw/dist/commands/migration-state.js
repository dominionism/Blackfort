...
const QUARANTINE_DIR_NAME = ".nemoclaw-quarantine";
...
function prepareSandboxState(snapshotDir, manifest) {
    ...
    quarantineImportedState(preparedStateDir, config, manifest);
...
function quarantineImportedState(preparedStateDir, config, manifest) {
    const quarantineDir = node_path_1.default.join(preparedStateDir, QUARANTINE_DIR_NAME);
    const quarantined = [];
    for (const entry of ["hooks", "extensions", "skills"]) {
        const sourcePath = node_path_1.default.join(preparedStateDir, entry);
        if (!(0, node_fs_1.existsSync)(sourcePath)) {
            continue;
        }
        (0, node_fs_1.mkdirSync)(quarantineDir, { recursive: true });
        const destinationPath = node_path_1.default.join(quarantineDir, entry);
        (0, node_fs_1.rmSync)(destinationPath, { recursive: true, force: true });
        (0, node_fs_1.renameSync)(sourcePath, destinationPath);
        quarantined.push({
    ...
    const skills = config["skills"] && typeof config["skills"] === "object" && !Array.isArray(config["skills"])
        ? config["skills"]
        : null;
    const skillLoad = skills?.["load"] && typeof skills["load"] === "object" && !Array.isArray(skills["load"])
        ? skills["load"]
        : null;
    if (skillLoad && Array.isArray(skillLoad["extraDirs"]) && skillLoad["extraDirs"].length > 0) {
        const quarantinedExtraDirs = skillLoad["extraDirs"]
            .filter((value) => typeof value === "string" && value.trim().length > 0)
            .map((value) => value.trim());
        if (quarantinedExtraDirs.length > 0) {
            for (const value of quarantinedExtraDirs) {
                quarantined.push({
                    kind: "skills-extra-dir",
                    name: "skills.load.extraDirs",
                    path: value,
                });
            }
        }
    ...
    if (quarantined.length > 0) {
        (0, node_fs_1.mkdirSync)(quarantineDir, { recursive: true });
        (0, node_fs_1.writeFileSync)(node_path_1.default.join(quarantineDir, "quarantine.json"), JSON.stringify({
            createdAt: new Date().toISOString(),
            reason: "Imported hooks, extensions, and extra skill directories require operator review before re-enablement.",
            entries: quarantined,
        }, null, 2));
    }
}
