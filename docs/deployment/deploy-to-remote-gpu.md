...
It now requires SSH host-key pinning through your `known_hosts` file or the `NEMOCLAW_SSH_KNOWN_HOSTS` environment variable.

## Prerequisites

- The [Brev CLI](https://brev.nvidia.com) installed and authenticated.
- An NVIDIA API key from [build.nvidia.com](https://build.nvidia.com).
- NemoClaw installed locally from a reviewed source tree or other pinned release source.
- `NEMOCLAW_OPENSHELL_SHA256` set to the expected OpenShell release archive digest if the remote bootstrap will install OpenShell for you.
...
4. Leaves Telegram ingress and remote browser access disabled until you explicitly configure them.
...
Before reconnecting from a new workstation or after rotating host keys, verify the pinned host key locally:
...
```console
$ ssh <instance-name> 'cd /home/ubuntu/nemoclaw && openshell term'
```
...
- [Run a Hardened NemoClaw Instance](hardened-instance-runbook.md) for the exact operator settings used by a hardened deployment.
