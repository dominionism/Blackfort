...
install_or_upgrade_ollama() {
  if detect_gpu && command_exists ollama; then
    ...
    if [[ -n "$current" ]] && version_gte "$current" "$OLLAMA_MIN_VERSION"; then
      ...
      error "Ollama bootstrap via remote install script is disabled. Upgrade Ollama manually from a trusted package source if you need local inference."
    fi
  else
    # No ollama — only install if a GPU is present
    if detect_gpu; then
      error "Ollama bootstrap via remote install script is disabled. Install Ollama manually from a trusted package source if you need local inference."
    else
...
install_nemoclaw() {
  if [[ -f "./package.json" ]] && grep -q '"name": "nemoclaw"' ./package.json 2>/dev/null; then
    ...
    error "Remote git installation is disabled for secure bootstrap. Clone the reviewed source tree locally and rerun this installer from the repository root."
  fi
...
verify_nemoclaw() {
  ...
  if [[ -n "$npm_bin" && -x "$npm_bin/nemoclaw" ]]; then
    ...
    warn "Try running the installer again from the reviewed source tree so it can perform a local install."
  fi
